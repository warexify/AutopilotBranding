...
if ("$env:PROCESSOR_ARCHITEW6432" -ne 'ARM64') {
	if (Test-Path "$env:SystemRoot\SysNative\WindowsPowerShell\v1.0\powershell.exe") {
		& "$env:SystemRoot\SysNative\WindowsPowerShell\v1.0\powershell.exe" -ExecutionPolicy Unrestricted -NoProfile -File "$PSCommandPath"
		Exit $lastexitcode
	}
}
...
if ($config.Config.Theme) {
	...
	Copy-Item "$PSScriptRoot\Autopilot.theme" "$env:SystemRoot\Resources\OEM Themes\Autopilot.theme" -Force
	...
	Copy-Item "$PSScriptRoot\DesktopBackground" 'C:\Windows\web\wallpaper\Autopilot\DesktopBackground' -Recurse -Force
	...
	Set-ItemProperty -Path $path -Name 'InstallTheme' -Type ExpandString -Value '%SystemRoot%\resources\OEM Themes\Autopilot.theme' -Force
}
